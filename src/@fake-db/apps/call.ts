  ...
  const filteredData = data.calls.filter(call => {
    if (dates.length) {
      ...
      if (filtered.length && filtered.includes(call.id)) {
        return (
          (call.companyEmail.toLowerCase().includes(queryLowered) ||
            call.name.toLowerCase().includes(queryLowered) ||
...
mock.onGet('/apps/call/clients').reply(() => {
  const clients = data.calls.map(call => {
    const { address, company, companyEmail, country, contact, name } = call
